...
router.post('/register', (req, res) => {
    ...
    try {
        const tests = [
            //removed for development        req.headers.referer === 'https://www.edenxi.com/tools' ||
            //removed for development       req.headers.referer === 'https://edenxi.com/tools', !disallowedIP.includes(req.headers['x-forwarded-for']),
            ...
            password.length >= 6 && password.length <= 15, !!username, //removed for development !!verify,
        ];
        ...
        if (!!tests.includes(false)) {
            ...
            request({
                ...
                async(error, resp) => {
                    ...
                    //removed for dev  if (resp.body.success) {
                    if (1 == 1) {
                        try {
                            ...
                            /*
                                                           ...
                                                           try {
                                                               const servercodequery =
                                                                   'SELECT code FROM code;';
                                                               const comparecode = await req.app.locals.query(servercodequery, [code]);
                                                               if (comparecode.length === 1) {


                                                                   //           Successful code query matches code form input
                                                                   // insert register success script here
                            ...
                            if (results.affectedRows) {
                                ...
                                return res.json({
                                    status: 'ERROR',
                                    errors: {
                                        server: 'Username already taken. Try something else.(1 no rows effected )',
                                    },
                                });
                            }
                        } catch (error) {
                            return res.json({
                                status: 'ERROR',
                                errors: {
                                    server: 'Username already taken. Try something else.(2 failed async function : accounts.js line 187 api/vi)',
                                },
                            });
                        }
                    } else {
