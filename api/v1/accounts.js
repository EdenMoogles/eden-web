...
router.get('/profile', validate, async (req, res) => {
    try {
        const statement =
            `SELECT *, IF(accounts_sessions.charid IS NULL, 0, 1) AS online FROM chars
...
router.post('/register', (req, res) => {
    ...
    try {
        ...
        if (!!tests.includes(false)) {
            res.json({
                status: 'ERROR',
                errors: { server: 'Your browser sent bad data. Please try again on a different browser.' },
            })
        } else {
                ...
                if (resp.body.success) {
                    try {
                        ...
                        if (results.affectedRows) {
                            ...
                            return res.json({
                                status: 'ERROR',
                                errors: { server: 'Username already taken. Try something else.' },
                            });
                        }
                    } catch (error) {
                        return res.json({
                    ...
                    return res.json({
                        status: 'ERROR',
                        errors: { server: 'Here be bots! You failed the ReCAPTCHA test. Make sure to fill out the form with valid data within a couple minutes.' },
                    });
                }
            });
        }
