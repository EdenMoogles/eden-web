...
const addExtraCharData = (chars) => {
    ...
    Object.keys(chars.online).forEach(charname => {
        ...
        online[charname] = Object.assign({}, char, { avatar: formatAvatar(char) });
    });
...
const getCharCrafts = async (query, charid) => {
    try {
        const statement =
            `SELECT skillid, value, rank FROM char_skills AS s
...
const getCharBazaar = async (query, charname) => {
    try {
        const statement =
            `SELECT b.name, i.bazaar FROM char_inventory AS i
...
const getCharEquip = async (query, charname) => {
    try {
        const statement =
            `SELECT equipslotid, b.itemid, b.name, i.signature FROM char_equip AS e
...
const getCharData = async (query, charname) => {
    try {
        const statement =
            `SELECT *, chars.charid AS charid, chars.accid AS accid, IF(accounts_sessions.charid IS NULL, 0, 1) AS isOnline FROM chars
...
const fetchChars = async (query, {
    ...
    try {
        const total = await query(`SELECT COUNT(*) AS ct FROM chars WHERE gmlevel = 0 AND charname LIKE ?;`, [`${search}%`]);
        const statement =
            `SELECT *, chars.charid AS charid, chars.accid AS accid, IF(accounts_sessions.charid IS NULL, 0, 1) AS isOnline FROM chars
            JOIN char_stats ON chars.charid = char_stats.charid
            JOIN char_look ON chars.charid = char_look.charid
